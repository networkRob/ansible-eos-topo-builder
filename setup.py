    ...
    for r1 in range(1,leaf+1):
        for r2 in range(1,racks+1):
            for r3 in range(1,3):
                ...
                l_vnic['vmnic'+str(cur_int+1)] = '%s-rack%s-leaf1_rack%s-leaf2'%(n_topo,r2,r2)
                leaf_topo['%s-rack%s-leaf%s'%(n_topo,r2,r3)] = l_vnic
    return(spine_topo,leaf_topo)

def build_veos_device(device,spine,leaf,power_on):
    ...
    if device == 'switch':
        ...
        if power_on:
            dict_veos.append({'name':"Powering on Spine Switches",
                            ...
                            'with_dict':"{{vm_config_spines}}"})
            dict_veos.append({'name':"Powering on Leaf Switches",
                            'delegate_to':'localhost',
                            'tags':'power-on_leafs',
                            'vsphere_guest':{
                                'vcenter_hostname':"{{esxi_ip}}",
...
def main():
    ...
    lab_domain = raw_input("What is the domain name for your lab? ").replace(' ','_')

    esxi_hostname = raw_input("What is the hostname of your ESXI instance? ")
    #esxi_ip = raw_input("What is the IP address for %s? "%esxi_hostname)
    esxi_ip = raw_input("What is the IP address for ESXI? ")
    ...
    base_def['domainname'] = lab_domain
...
# =================================================
    ...
    with open(lab_yaml,'w') as lab_f:
        ...
        #lab_f.write('ansible_user=%s\n'%ans_user)
        #lab_f.write('ansible_ssh_pass=%s\n\n'%ans_pwd)
    ...
    with open(roles_veos_switches,'w') as v_switches:
        export_yaml(build_veos_device('switch',num_spines,num_leafs,vm_state),v_switches)
